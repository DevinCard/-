...
window.addTransactionToDisplay = function(transaction) {
    ...
    // Check if the category contains an emoji (custom category)
    let emoji;
    if (transaction.category.includes(' ')) {
        emoji = transaction.category.split(' ')[0];
        transaction.category = transaction.category.split(' ')[1];
    } else {
        const emojiMap = {
            Food: "🍔",
            Transport: "🚌",
        ...
        emoji = emojiMap[transaction.category] || "💰";
    }
    ...
    menuButton.onclick = () => showEditModal(transaction);

    // Combine elements
    transactionElement.appendChild(transactionContent);
    transactionElement.appendChild(menuButton);
    transactionElement.setAttribute("data-category", transaction.category);
...
window.filterTransactions = async function() {
    ...
    try {
        ...
        transactions.forEach(transaction => {
            ...
            if (transaction.category.includes(' ')) {
                // For custom categories, split emoji and name
                categoryName = transaction.category.split(' ')[1];
            }
